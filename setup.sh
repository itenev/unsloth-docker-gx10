...
# Resolve actual version if "latest" so image tag is always specific
if [[ "${UNSLOTH_VERSION}" == "latest" ]]; then
  RESOLVED_VERSION=$(pip index versions unsloth 2>/dev/null |     grep -oP "(?<=Available versions: )[^,]+" | head -1 || echo "latest")
  IMAGE_TAG="${RESOLVED_VERSION}"
else
  IMAGE_TAG="${UNSLOTH_VERSION}"
fi
IMAGE_NAME="unsloth-gx10:${IMAGE_TAG}"
...
echo " Unsloth version  : ${UNSLOTH_VERSION} (resolved: ${IMAGE_TAG})"
